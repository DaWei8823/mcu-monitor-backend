...
HEADER_MESSAGE_TYPE_FIELD_END = HEADER_MESSAGE_TYPE_FIELD_START + HEADER_MESSAGE_TYPE_FIELD_SIZE


def parse_client_header(raw : bytearray):
    if len(raw) < HEADER_TOTAL_LENGTH:
        raise ValueError(f"expcted header of length {HEADER_TOTAL_LENGTH} bytes instead got {len(raw)} bytes")
    ...
    message_type_slice = raw[HEADER_MESSAGE_TYPE_FIELD_START:HEADER_MESSAGE_TYPE_FIELD_END]
